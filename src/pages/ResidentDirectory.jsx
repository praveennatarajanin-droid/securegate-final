...
export default function ResidentDirectory() {
    ...
    const fetchResidents = async () => {
        try {
            ...
            const response = await apiService.getAllResidents(user.society_id);
    ...
    const fetchBlocks = async () => {
        try {
            const user = JSON.parse(localStorage.getItem('user') || '{}');
            const response = await apiService.getAllBlocks(user.society_id);
            if (response && response.data) {
    ...
    const openAddModal = () => {
        ...
        setCurrentResident({ 
            ...
            phone: '', 
            email: '', 
            block: blocks.length > 0 ? blocks[0].name : 'Block A', 
